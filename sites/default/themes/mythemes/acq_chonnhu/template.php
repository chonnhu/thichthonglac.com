 ...
 */
function acq_chonnhu_preprocess_html(&$vars) {
  $vars['classes_array'][] = 'theme-markup-2';
  $vars['classes_array'][] = _acq_chonnhu_get_layout();
}

/**
 * Implements hook_html_head_alter().
 */
function acq_chonnhu_html_head_alter(&$head_elements) {
...
function acq_chonnhu_preprocess_page(&$variables) {
  ...
  $layoutClass = _acq_chonnhu_get_layout();
 ...
 */
function acq_chonnhu_preprocess_block(&$vars) {
 ...
 */
function _acq_chonnhu_variable_get($key, $default) {
  global $theme;
 ...
 */
function _acq_chonnhu_get_layout() {
  $layout_patterns = _acq_chonnhu_variable_get('layout', array('<global>' => 'body-layout-fixed-abc'));
  ...
  foreach ($layout_patterns as $pattern => $layout) {
    // DG-3784 the path is saved in the .info file urlencoded.
    // If it's not urldecoded, matches to e.g. accented characters fail.
    if (drupal_match_path($alias_path, rawurldecode($pattern)) ||
 ...
 */
function acq_chonnhu_node_view_alter(&$build) {
...
function acq_chonnhu_preprocess_forum_topic_list(&$vars) {
 ...
 */
function acq_chonnhu_preprocess_comment_wrapper(&$vars) {
 ...
 */
function acq_chonnhu_preprocess_media_gallery_license(&$vars) {
